...
export const AddTorrentModal: React.FC<AddTorrentModalProps> = ({
	...
	const inspectFile = useCallback(async (path: string) => {
		...
		setError("");
		try {
			const res = await FileService.InspectTorrentFile(path);
			if (res) {
				setInspectData(res as unknown as TorrentInspectData);
			}
		} catch (err: unknown) {
			console.error("Failed to inspect torrent file:", err);
			setInspectData(null);
		} finally {
			setInspecting(false);
		}
	...
	return (
		<div className="fixed inset-0 modal-overlay flex items-center justify-center z-50 p-4">
			<div className="modal-card max-w-2xl w-full p-6 rounded-2xl shadow-2xl space-y-5 max-h-[90vh] flex flex-col overflow-hidden">
				{/* Modal Header */}
				...
				>
						...
						<div>
							...
							<div className="flex gap-2">
								...
								<button
									...
									className="px-3.5 py-2 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-semibold rounded-xl shadow-md shadow-indigo-600/20 transition-all flex items-center gap-1.5 shrink-0"
								>
					...
					{/* Loading State during Inspection */}
					{inspecting && (
						<div className="p-4 bg-slate-900/60 rounded-xl border border-slate-800 flex items-center justify-center gap-2 text-xs text-slate-400">
							...
							<span>Reading torrent metainfo details...</span>
						</div>
					)}

					{/* Detailed Torrent Metadata View */}
									...
									<div className="space-y-3 text-xs">
										...
										<div className="p-2.5 bg-slate-950/60 rounded-lg border border-slate-800 flex items-center justify-between gap-2">
											<div className="min-w-0 flex-1 font-mono">
												<span className="text-[10px] text-slate-400 block flex items-center gap-1">
													<Hash className="w-3 h-3 text-purple-400" /> Info Hash
												</span>
										...
										<div className="space-y-1.5 text-[11px]">
											...
											<div className="flex items-center justify-between text-slate-400">
												...
												<span className="text-slate-300 font-mono truncate max-w-[300px]">
													{inspectData.sourceFilePath}
												</span>
											</div>
										</div>
									</div>
									...
									<div className="space-y-1.5 font-mono text-[11px]">
										{inspectData.trackers?.length > 0 ? (
											...
											<p className="text-slate-500 text-xs italic text-center py-4">
												No public trackers listed in torrent info.
											</p>
										)}
									</div>
								)}
